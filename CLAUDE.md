...
No dependencies beyond Python 3.11+ stdlib. Test suites: `tests/test_report.py` (529 tests for the main script's pure functions), `hooks/test_block_secrets.py` (76 tests), `hooks/test_bypass_fixes.py` (110 tests), `hooks/test_round2_bypass_fixes.py` (50 tests), `hooks/test_round3_bypass_fixes.py` (82 tests), `hooks/test_analysis_fixes.py` (68 tests), `hooks/test_fp_fixes.py` (58 tests), `hooks/test_infra_usability.py` (139 tests), `hooks/test_warn_secrets.py` (15 tests), `hooks/test_warn_mode.py` (39 tests), `hooks/test_warn_output_adversarial.py` (48 tests), `hooks/test_phase1_audit_fixes.py` (31 tests), `hooks/test_round4_bypass_fixes.py` (26 tests), `hooks/test_phase3_audit_fixes.py` (20 tests), `hooks/test_phase4_audit_fixes.py` (78 tests), `hooks/test_phase5_audit_fixes.py` (24 tests), `hooks/test_phase6_audit_fixes.py` (17 tests), `hooks/test_phase7_audit_fixes.py` (9 tests), `scripts/check-pattern-sync.py` (10 checks) — 1429 total across 19 suites.
...
3. **Risk classification** (`classify_risk`) — categorizes each tool call as destructive/mutating/read-only. Bash commands get sub-command-aware logic for git, curl, find, sed, ansible-playbook. Compound commands are split on top-level `;`/`&&`/`||` (`_split_top_level_segments`, quote/subshell-aware) and classified at the worst segment's risk (`_max_risk`), so a benign leading command can't mask a dangerous trailing one (e.g. `true && git push --force` → destructive). `source`/`timeout` runner dispatch takes precedence over the generic split. Secret detection runs 22 provider-specific token regexes plus Shannon entropy on base64 blobs (primary >= 3.5 threshold plus secondary unique-char-ratio check for 3.0-3.5 range to catch padding evasion)
...
| Residual entropy/secret_assign FPs (#9: `IDENT=` prefix, digit-bearing segments, camelCase-with-digit, `+`-joined lists, env-read + trailing `;`) | 2026-06-20 | Fixed in both report + hook (shared `_is_benign_high_entropy`); D left as documented trade-off | [`docs/history/report-entropy-fp-2026-06-20.md`](docs/history/report-entropy-fp-2026-06-20.md) |
| 12-dimension project audit (2 H, 8 M, 13 L, 9 I) | 2026-05-22 | H1 closed in v1.1.2 (Ship A); H2 + M4 + M5 + #758 closed in v1.2.0 (Ship B); M6 + M7 + M8 + L2 + L10 + L11 + L12 closed in v1.2.1 (Ship C); M2 + L-cap + L-safe + L-idem + L-doc closed in v1.2.2 (Ship D); M1 / 2.1 realpath pre-filter measured-and-retained 2026-05-22 (2.45× speedup vs. symlink-coverage loss, benchmark at `scripts/bench-realpath.py`) | [`docs/history/audit-2026-05-22.md`](docs/history/audit-2026-05-22.md) |
